...
const Profile = () => {
  ...
  const {email, logout, getProfile, token, user } = useContext(UserContext);
  

  // 3 se obtiene la información del perfil al cargar el componente
  useEffect(() => {
    if (token) {
      getProfile();
    }
  ...
  const handleLogout = () => {
    ...
    logout();// limpia el estado del token y el email
    // Redirigir a login
    navigate("/login");
  };
          ...
          <Col md={6} lg={5}>
            <Card style={styles.card}>
              ...
              <Card.Body className="text-center pb-5">
                ...
                <h3 className="mt-5 mb-1 fw-bold">Usuario Mamma Mía</h3>
                <p className="text-muted mb-3">{email || user?.email}</p>
                
                <div className="d-flex justify-content-center gap-2 mb-4">
                  <Badge bg="warning" text="dark">Cliente verificado</Badge>
                  <Badge bg="dark"> Pedidos:</Badge>
                </div>
              ...
              <Card.Footer className="bg-white border-0 text-center pb-4">
                <small className="text-muted">Fecha de registro: {user?.createdAt || "No disponible"}</small>
              </Card.Footer>
            </Card>
          </Col>
        </Row>
